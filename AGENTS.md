...
- **Walker & watcher** — `packages/core/src/fs/`. Walker (`walker.ts`) + watcher (`watcher.ts`) feed the files-tab tree. Respects `.gitignore` + `.git/info/exclude`. CRUD ops in `ops.ts`; deletes go through `shell.trashItem` (injected from the desktop bridge via `setTrashImpl`). All paths validated against project root to block traversal.
- **File tree UI** — `packages/ui/src/features/files/`. Virtualized tree (`@tanstack/react-virtual`), fuzzy filter (`fuse.js`). File-type icons reuse `iconForFile()` from `packages/ui/src/components/icons/file-icons.ts` (iconify + `material-icon-theme`); folder rows use Lucide `Folder` / `FolderOpen`. Drag payload uses the `application/x-pideck-paths` MIME and feeds the composer attachment pipeline via `useComposerPathDrop` (shared by `MessageInput.tsx` and `PidComposerScreen.tsx`).
