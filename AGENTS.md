...
- **Theme system** — `packages/ui/src/theme/`. Tokens defined in `tokens.css` using descriptive names (`--bg-0..3`, `--ink-0..3`, `--accent*`, `--add/del/mod`, `--diff-*`). Active theme is JSON, applied by `loader.ts` as inline custom properties on `<html>`. Shiki bridge in `shiki-bridge.ts` keeps syntax highlighting aligned with the active theme — when a VS Code theme is active the bridge feeds Shiki the original VS Code JSON for key-for-key tokenisation; otherwise it picks a bundled Shiki theme by light/dark kind. Six bundled palettes: default / phosphor / nightshade × dark / light. The canonical Zod schema lives in `packages/core/src/protocol/theme.ts` and is re-exported via `@pi-deck/core`.
- **Theme storage & hot reload** — `packages/core/src/host/themes/`. Bundled JSON lives in `packages/core/src/host/themes/bundled/` and is imported with `with { type: "json" }`. User themes live at `<userData>/themes/` (Electron `app.getPath("userData")`). VS Code colour themes are translated to a full pi-deck `ThemeSpec` on disk-read via `vscode-adapter.ts` (in core, not the renderer) so imported themes actually drive surface/ink/accent — not just light/dark. The raw VS Code JSON is preserved alongside the spec and round-tripped through `theme.get` so the renderer can hand it to Shiki. Chokidar watches the dir and emits `theme.changed` events; if the active theme's spec changed on disk, the new spec ships in the event payload so the renderer re-applies without a round-trip. User themes can be deleted via the `theme.delete` command — the host removes the file from disk and falls back to `default-dark` if the deleted theme was active.
...
- **Artefacts tracking** — `packages/core/src/host/artefacts-tracker.ts`. Sister to the turn tracker, but narrower: only fires for `write` / `create` / `create_file` / `file_write` tools, and stashes `existsBefore` on `session.tool.call.start` so the post-call register step can filter out *edits of existing files*. Emits `session.artefacts.changed` with `{ path, sizeBytes, createdAt }` per newly-created file. The renderer also primes from `session.artefacts.list` when the Context tab mounts. In-memory; forgotten on deactivate / delete / worker exit.
- **Context tab** — `packages/ui/src/features/context/`. Right-pane tab that surfaces (1) the segmented context-window bar driven by `contextBreakdown.ts` (the same helper the composer's `ContextUsageIndicator` ring tooltip uses), (2) "in scope" — deduped attachments aggregated from `UserMessageEntry.attachments` in the current session, (3) "artefacts produced" — `useArtefactsStore` rows plus the session's plan-mode markdown when present. Row actions route to `window.bridge.openPath` (system default app) and `window.bridge.showItemInFolder` (reveal in file manager).
