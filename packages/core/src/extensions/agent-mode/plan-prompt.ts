...
function planSection(planFilePath: string): string {
  return [
    ...
    "",
    "When the request is clear enough, produce a plan as your final assistant message. Begin with",
    "a single H1 title line - `# <short imperative title>` naming what the plan accomplishes -",
    ...
    "tool. Overwrite the file in place — it represents the *current* plan. If the directory does",
    "not exist yet, the write tool will create it.",
    "",
    ...
    "",
    "After approval, follow that note - keep the plan file's checkboxes current as you work so the",
